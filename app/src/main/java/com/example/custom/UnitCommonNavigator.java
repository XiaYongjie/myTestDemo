 ...
 */
public class UnitCommonNavigator extends FrameLayout implements IPagerNavigator, NavigatorHelper.OnNavigatorScrollListener {
    private HorizontalScrollView mScrollView;
    private LinearLayout mTitleContainer;
    private LinearLayout mTitleBgContainer;
    ...
    public UnitCommonNavigator(Context context) {
        super(context);
    ...
    private void init() {
        ...
        View root;
        root = LayoutInflater.from(getContext()).inflate(R.layout.temp_pager_navigator_layout, this);
        mScrollView = (HorizontalScrollView) root.findViewById(R.id.scroll_view);   // mAdjustMode为true时，mScrollView为null
        mTitleBgContainer = root.findViewById(R.id.title_container_bg);
        mTitleBgContainer.setPadding(mLeftPadding, 0, mRightPadding, 0);
...
//        }
    ...
    private void initTitlesAndIndicator() {
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            ...
            if (v instanceof UnitPagerTitleView){
                UnitBgView bg =new UnitBgView(getContext());
                bg.setText(((UnitPagerTitleView) v).getText());
                mTitleBgContainer.addView(bg);
            }
        }
        if (mAdapter != null) {
            mIndicator = mAdapter.getIndicator(getContext());
            if (mIndicator instanceof View) {
                mIndicatorContainer.addView((View) mIndicator);
            }
        }
    }
...
//                    bgView.setLayout(view.getContentLeft(),view.getContentTop(),view.getContentRight(),view.getContentBottom());
                } else {
